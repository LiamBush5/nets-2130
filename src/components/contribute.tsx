...
export function ContributeComponent() {
    ...
    const fetchIncompleteSlangTerms = useCallback(async () => {
        if (!user?.generation?.generation_name) {
            console.log('User or generation data not available')
            return
        }
                    ...
                    <div className="mt-8">
                        ...
                        <ul className="list-disc pl-5 space-y-2">
                            {incompleteSlangTerms.map((term) => (
                                <li key={term.slang_id} className="hover:bg-gray-50 p-2 rounded">
                                    <span
                                    ...
                                    <div className="mt-1 text-sm text-gray-600">
                                        {term.definition_gen_z && (
                                            <div>Gen Z: {term.definition_gen_z}</div>
                                        )}
                                        {term.definition_millennials && (
                                            <div>Millennials: {term.definition_millennials}</div>
                                        )}
                                        {term.definition_gen_x && (
                                            <div>Gen X: {term.definition_gen_x}</div>
                                        )}
                                    </div>
                                </li>
                            ))}
